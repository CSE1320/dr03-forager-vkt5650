...
export default function MushroomInfoSection({ name, scientificName, fastFacts }) {
  ...
  const handleAddToFavorites = () => {
    ...
    if (storedFavorites.includes(mushroomData.id)) {
      updatedFavorites = storedFavorites.filter(id => id !== mushroomData.id); // Remove from favorites
    } else {
      updatedFavorites = [...storedFavorites, mushroomData.id]; // Add to favorites
    }
